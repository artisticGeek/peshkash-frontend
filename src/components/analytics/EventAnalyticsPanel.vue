<template>
  <div class="scoped-analytics-panel">
    ...
    <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-3">
      <div>
        <h6 class="fw-bold mb-0">
          <i class="bi bi-bar-chart-line me-2 text-primary"></i>Analytics — {{ eventName }}
        </h6>
    ...
    <div v-else-if="!summary || summary.totalScans === 0" class="text-center py-4 text-muted">
      ...
      <p class="mb-0 small">Analytics will appear after customers scan this event's QR.</p>
    </div>
    ...
    <template v-else-if="summary">
      ...
      <p class="summary-sentence mb-3">
        ...
        <span v-if="topItem"> The most viewed item was <strong>{{ topItem.itemName }}</strong>.</span>
      ...
      <div class="row g-2 mb-3">
        ...
        <div class="col-6 col-md-3">
          <KpiCard label="Menu Views" :value="actionCount('menu_view')" icon="bi-layout-three-columns" icon-class="text-info" />
        </div>
        <div class="col-6 col-md-3">
          <KpiCard label="Item Views" :value="itemViewCount" icon="bi-eye" icon-class="text-warning" />
        </div>
        <div class="col-6 col-md-3">
...
interface QrDetail { qrHash: string; qrType: string; targetName: string; scans: number; actions: number; }
interface ItemViewed { itemId: number; itemName: string; views: number; }
interface Summary {
...
const itemViewCount = computed(() =>
  (actionCount('item_expand') + actionCount('item_detail_view'))
);

const topItem = computed(() => summary.value?.topItemsViewed?.[0] ?? null);
...
.rank-num { display: inline-flex; align-items: center; justify-content: center; width: 18px; height: 18px; border-radius: 50%; background: var(--bs-light); font-size: 0.65rem; font-weight: 700; color: var(--bs-secondary); flex-shrink: 0; }
