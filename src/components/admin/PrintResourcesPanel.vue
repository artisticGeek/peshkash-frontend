  ...
  <div class="brochure-library">
    <section class="library-hero">
      <div class="hero-copy">
        ...
        <p class="hero-intro">A growing collection of press-ready ways to show how Peshkash adds a current, measurable digital layer to products, counters, cards and real-world conversations.</p>
      </div>
    ...
    <section class="brand-kit-section" aria-labelledby="brand-kit-heading">
      <div class="brand-kit-heading">
        <div>
          <p class="eyebrow">Brand kit assets</p>
          <h2 id="brand-kit-heading">The essentials, ready to use.</h2>
        </div>
        <p>Download the approved logo files and the current brand guidelines before creating new Peshkash material.</p>
      </div>

      <div class="brand-kit-grid">
        <article v-for="asset in brandAssets" :key="asset.id" class="brand-kit-card">
          <div class="brand-kit-preview" :class="`brand-kit-preview--${asset.id}`">
            <img :src="asset.preview" :alt="`${asset.title} preview`" loading="lazy" />
            <span>{{ asset.number }}</span>
          </div>
          <div class="brand-kit-copy">
            <p class="brand-kit-type">{{ asset.type }}</p>
            <h3>{{ asset.title }}</h3>
            <p>{{ asset.description }}</p>
            <div class="brand-kit-tags"><span v-for="tag in asset.tags" :key="tag">{{ tag }}</span></div>
            <div class="brand-kit-actions">
              <a :href="asset.download" :download="fileName(asset.download)"><i class="bi bi-file-earmark-zip"></i> Download full pack</a>
              <a v-if="asset.guide" :href="asset.guide" target="_blank" rel="noopener"><i class="bi bi-file-earmark-pdf"></i> Open PDF</a>
            </div>
          </div>
        </article>
      </div>
...
const brandAssets: BrandAsset[] = [
  {
    ...
    number: '01',
    title: 'Logo',
    type: 'Approved identity files',
    description: 'Primary marks, symbols, app icons and source exports for light and dark backgrounds.',
    tags: ['SVG', 'PNG', 'App icons'],
    preview: '/resources/brand-kit/Peshkash-01-Logo-preview.png',
    download: '/resources/brand-kit/Peshkash-01-Logo.zip',
  },
  {
    id: 'guidelines',
    number: '02',
    title: 'Guidelines',
    type: 'Brand rules and references',
    description: 'The approved colour palette, typography, clear space and practical rules for consistent use.',
    tags: ['PDF', 'Colour', 'Typography'],
    preview: '/resources/brand-kit/Peshkash-02-Guidelines-preview.png',
    download: '/resources/brand-kit/Peshkash-02-Guidelines.zip',
    guide: '/resources/brand-kit/Peshkash-Brand-Guidelines.pdf',
  },
];
...
/* Brand kit downloads */
.brand-kit-section{background:#fff;border:1px solid var(--line);padding:clamp(1.5rem,3vw,2.5rem)}
.brand-kit-heading{align-items:end;display:grid;gap:2rem;grid-template-columns:minmax(0,1fr) minmax(260px,.7fr);margin-bottom:1.5rem}
.brand-kit-heading .eyebrow{margin-bottom:.55rem}.brand-kit-heading h2{font-family:'Rufina',Georgia,serif;font-size:clamp(2rem,4vw,3.25rem);font-weight:400;letter-spacing:-.035em;line-height:1;margin:0}.brand-kit-heading>p{color:#75665b;font-size:.78rem;line-height:1.6;margin:0}
.brand-kit-grid{display:grid;gap:1rem;grid-template-columns:repeat(2,minmax(0,1fr))}.brand-kit-card{border:1px solid var(--line);display:grid;grid-template-columns:minmax(180px,.72fr) minmax(0,1.28fr);min-width:0}.brand-kit-preview{align-items:center;background:#f2ece5;display:flex;justify-content:center;min-height:245px;overflow:hidden;padding:1.4rem;position:relative}.brand-kit-preview img{display:block;height:auto;max-height:170px;max-width:100%;object-fit:contain}.brand-kit-preview>span{background:var(--ink);color:var(--paper);font-family:'Rufina',serif;font-size:1.05rem;left:.65rem;padding:.45rem;position:absolute;top:.65rem}.brand-kit-preview--guidelines{background:#e9ded2}.brand-kit-preview--guidelines img{box-shadow:0 12px 28px rgba(26,20,16,.12)}
.brand-kit-copy{display:flex;flex-direction:column;padding:1.2rem}.brand-kit-type{color:#9a6f3d;font-size:.55rem;letter-spacing:.14em;margin:0;text-transform:uppercase}.brand-kit-copy h3{font-family:'Rufina',serif;font-size:1.65rem;font-weight:400;margin:.5rem 0}.brand-kit-copy>p:not(.brand-kit-type){color:#75665b;font-size:.7rem;line-height:1.5;margin:0}.brand-kit-tags{display:flex;flex-wrap:wrap;gap:.3rem;margin-top:.8rem}.brand-kit-tags span{background:#f1e8dd;color:#745638;font-size:.5rem;letter-spacing:.08em;padding:.28rem .42rem;text-transform:uppercase}.brand-kit-actions{display:flex;flex-wrap:wrap;gap:.45rem;margin-top:auto;padding-top:1rem}.brand-kit-actions a{align-items:center;background:var(--ink);color:var(--paper);display:inline-flex;font-size:.63rem;gap:.4rem;padding:.62rem .72rem;text-decoration:none}.brand-kit-actions a+a{background:#f4eee7;border:1px solid #d9c5ae;color:var(--ink)}
@media(max-width:1250px){.brochure-grid{grid-template-columns:1fr}.brochure-card{grid-template-columns:minmax(220px,.7fr) minmax(0,1.3fr)}.brand-kit-card{display:block}}
@media(max-width:760px){.library-hero{display:block;min-height:0;padding:2rem 1.25rem}.library-hero h1{font-size:3.1rem}.hero-proof{margin-top:1.6rem}.brand-kit-heading{display:block}.brand-kit-heading>p{margin-top:1rem}.brand-kit-grid{grid-template-columns:1fr}.brand-kit-preview{min-height:210px}.library-toolbar{align-items:stretch;flex-direction:column}.library-total{justify-content:center}.brochure-card{display:block}.brochure-preview{height:340px;min-height:0}.file-actions{grid-template-columns:1fr 1fr}.press-note{display:block;padding:2rem 1.25rem}.press-note ol{display:block;margin-top:1.5rem}.press-note li{border-left:0;border-top:1px solid rgba(187,144,87,.3);padding:.9rem 0}}
</style>
