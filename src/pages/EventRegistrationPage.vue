  ...
  <div class="event-page">
    ...
    <main v-else class="event-content">
      ...
      <section v-if="event.organizer" class="event-section organizer-card">
        ...
        <p v-if="event.organizer.description">{{ event.organizer.description }}</p>
        <div v-if="event.experience.contactVisible && event.organizer.contact?.length" class="organizer-links">
          <a v-for="contact in event.organizer.contact" :key="String(contact)" :href="contactHref(String(contact))" @click="track('event_organizer_contact_click')"><i :class="contactIcon(String(contact))"></i>{{ contact }}</a>
        </div>
        <RouterLink v-if="event.organizer.name" :to="`/vendor/${event.organizer.name}`">Know the organizer <i class="bi bi-arrow-right"></i></RouterLink>
      </section>
...
function contactHref(value: string) { const trimmed = value.trim(); return trimmed.includes('@') && !trimmed.startsWith('http') ? `mailto:${trimmed}` : /^\+?[\d\s()-]{7,}$/.test(trimmed) ? `tel:${trimmed.replace(/\s/g, '')}` : trimmed; }
function contactIcon(value: string) { return value.includes('@') && !value.startsWith('http') ? 'bi bi-envelope' : /^\+?[\d\s()-]{7,}$/.test(value.trim()) ? 'bi bi-telephone' : 'bi bi-link-45deg'; }
