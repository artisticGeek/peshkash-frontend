<template>
          ...
          <div v-else>
            <nav class="mb-2 mb-md-3" aria-label="breadcrumb">
              <ol class="breadcrumb small mb-0">
                <li class="breadcrumb-item"><a href="#" class="link-secondary">{{ itemData.event.displayName }}</a></li>
                <li v-for="parent in itemData.parentItems || []" :key="parent.displayName" class="breadcrumb-item">
                  <a href="#" class="link-secondary">{{ parent.displayName }}</a>
                </li>
            ...
            <div class="card border-0 shadow rounded-4 overflow-hidden pk-reveal" data-anim="fadeInUp">
              ...
              <div class="card-body pb-0">
                <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-md-between gap-2">
                  <div>
                    <h2 class="h3 h2-md mb-0 text-body-emphasis">{{ itemData.name }}</h2>
                    <div class="small text-uppercase text-secondary">{{ itemData.event.vendor.displayName }}</div>
                  </div>
                  <div v-if="itemData.price" class="fw-semibold text-body">{{ itemData.price }}</div>
                </div>

                <div class="d-flex flex-wrap align-items-center gap-2 mt-3">
                  <span v-if="itemData.isVeg !== undefined" class="d-flex align-items-center me-2 mb-2">
                    <svg width="8" height="8" :class="itemData.isVeg ? 'text-success' : 'text-danger'" aria-hidden="true" focusable="false">
                      <circle cx="4" cy="4" r="4" fill="currentColor" />
                    </svg>
                    <span class="ms-1 small text-muted">{{ itemData.isVeg ? 'Veg' : 'Non-Veg' }}</span>
                  </span>
                  <span v-for="tag in itemData.tags || []" :key="'tag'+tag" class="badge rounded-pill text-bg-light border me-2 mb-2">{{ tag }}</span>
                  <span v-for="allergen in itemData.allergens || []" :key="'allergen'+allergen" class="badge rounded-pill text-bg-light border me-2 mb-2">{{ allergen }}</span>
                  <span v-if="itemData.spiceLevel" class="d-flex align-items-center mb-2">
                    <svg
                      ...
                      fill="currentColor"
                      class="me-1"
                      :class="n <= itemData.spiceLevel ? 'text-warning' : 'text-secondary'"
                      aria-hidden="true"
                      focusable="false"
                    >
                ...
                <div class="ratio ratio-16x9 rounded-3 shadow-sm my-3 my-md-4 pk-reveal" data-anim="fadeInUp">
                  <img
                ...
                <div class="px-3 px-md-4 pb-4 pk-reveal" data-anim="fadeInUp">
                  <h3 class="h5 text-warning d-flex align-items-center mb-2">
                    <svg
                  ...
                  <p class="lead fs-6 text-body-secondary mb-3">{{ itemData.description }}</p>
                  <div class="border-top my-3"></div>
                  <div v-if="itemData.ingredients" class="d-flex flex-wrap">
                    <span
                      v-for="ing in itemData.ingredients.split(',')"
                      :key="ing"
                      class="badge rounded-pill text-bg-light border me-2 mb-2"
                    >
                      {{ ing.trim() }}
                    </span>
                  </div>

                  <div class="text-center text-muted mb-2 small text-uppercase mt-4">Rate this item</div>
                  <div class="d-flex justify-content-center">
                    <div class="btn-group" role="radiogroup">
                      <template v-for="n in 5">
                        <input class="btn-check" type="radio" name="rating" :id="'star'+n" />
                        <label class="btn btn-outline-warning" :for="'star'+n">
                  ...
                  <div class="d-flex gap-2 justify-content-center mt-3">
                    <button class="btn btn-outline-success">
                      Like
                      <span class="badge text-bg-light ms-1">{{ itemData?.likes || 0 }}</span>
                    </button>
                    <button class="btn btn-outline-danger">
                      Dislike
                      <span class="badge text-bg-light ms-1">{{ itemData?.dislikes || 0 }}</span>
                    </button>
                  </div>
                  <button
  ...
  <div class="fixed-bottom bg-body border-top shadow-sm d-md-none">
