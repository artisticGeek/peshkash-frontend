    ...
    <main class="uh-shell">
      ...
      <template v-else>
        ...
        <template v-else-if="isPreferencesPage">
          <section class="uh-heading"><div><p class="uh-section-kicker">Updates</p><h2>Your messages</h2></div><p>{{ updates.length }} {{ updates.length===1?'update':'updates' }}</p></section>
          <section v-if="updates.length" class="uh-updates-list"><RouterLink v-for="update in updates" :key="update.id" :to="update.destinationPath" class="uh-update-card"><span :class="['uh-update-icon',{'is-whatsapp':update.channel==='whatsapp'}]"><i :class="update.channel==='whatsapp'?'bi bi-whatsapp':'bi bi-bell'"></i></span><span class="uh-update-copy"><span><strong>{{ update.title }}</strong><time>{{ relativeTime(update.occurredAt) }}</time></span><p>{{ update.message }}</p><small>{{ update.vendorName }} · Tap to open</small></span><i class="bi bi-chevron-right"></i></RouterLink></section>
          <section v-else class="uh-empty"><i class="bi bi-bell"></i><div><h2>No updates yet</h2><p>Messages sent to you by Peshkash will appear here.</p></div></section>
          <section class="uh-heading uh-settings-heading"><div><p class="uh-section-kicker">Preferences</p><h2>How updates reach you</h2></div></section>
          <p class="uh-preferences-intro">These choices apply across your account.</p>
          <div class="uh-settings-card">
            <label class="uh-setting-row">
              <span class="uh-setting-icon"><i class="bi bi-bell"></i></span>
              <span class="uh-setting-copy"><strong>App notifications</strong><span>{{ notificationStatusCopy }}</span></span>
              <span class="uh-switch"><input type="checkbox" :checked="settings.pushEnabled" :disabled="settingsBusy !== null" @change="togglePush(($event.target as HTMLInputElement).checked)" /><span></span><em>{{ settings.pushEnabled ? 'On' : 'Off' }}</em></span>
            </label>
            <label class="uh-setting-row">
              <span class="uh-setting-icon is-whatsapp"><i class="bi bi-whatsapp"></i></span>
              <span class="uh-setting-copy"><strong>WhatsApp updates</strong><span>Occasional messages from Peshkash about relevant collections and events.</span></span>
              <span class="uh-switch"><input type="checkbox" :checked="settings.whatsappEnabled" :disabled="settingsBusy !== null" @change="toggleWhatsApp(($event.target as HTMLInputElement).checked)" /><span></span><em>{{ settings.whatsappEnabled ? 'On' : 'Off' }}</em></span>
            </label>
          </div>
...
interface CommunicationSettings { whatsappEnabled:boolean;pushEnabled:boolean }
interface UserUpdate { id:number;title:string;message:string;channel:'push'|'whatsapp';destinationPath:string;vendorName:string;occurredAt:string }
const auth=useAuthStore(),route=useRoute(),router=useRouter()
const loginOpen=ref(false),loading=ref(false),error=ref(''),history=ref<History|null>(null),search=ref(''),range=ref<'7d'|'30d'|'90d'|'all'>('30d'),audit=ref<Audit>({rows:[],total:0,page:1,pageSize:20,pages:1}),failedImages=ref(new Set<string>()),settings=ref<CommunicationSettings>({whatsappEnabled:false,pushEnabled:false}),settingsBusy=ref<'whatsapp'|'push'|null>(null),preferenceMessage=ref('')
const updates=ref<UserUpdate[]>([])
...
const notificationStatusCopy=computed(()=>{
  if(!pushSupported())return'Notifications are not supported on this device.'
  if(Notification.permission==='denied')return'Blocked in this device’s system settings.'
  return'Relevant collection and event updates from Peshkash.'
})
...
async function loadPreferences(){const[settingsResponse,updatesResponse]=await Promise.all([axios.get<CommunicationSettings>(`${API_BASE_URL}/user/communication-settings`),axios.get<{updates:UserUpdate[]}>(`${API_BASE_URL}/user/updates`)]);settings.value=settingsResponse.data;updates.value=updatesResponse.data.updates||[]}
...
async function toggleWhatsApp(enabled:boolean){settingsBusy.value='whatsapp';preferenceMessage.value='';const previous=settings.value.whatsappEnabled;settings.value.whatsappEnabled=enabled;try{await axios.put(`${API_BASE_URL}/user/communication-settings/whatsapp`,{enabled});preferenceMessage.value=enabled?'WhatsApp updates are on.':'WhatsApp updates are off.'}catch(e:any){settings.value.whatsappEnabled=previous;preferenceMessage.value=e?.response?.data?.error||'Could not update WhatsApp settings.'}finally{settingsBusy.value=null}}
...
<style scoped>
.uh-preferences-intro{color:#786858;font-size:.82rem;line-height:1.55;max-width:650px}.uh-settings-card{background:#fffaf3;border:1px solid #ded2c4;border-radius:16px;margin-top:1.2rem;overflow:hidden}.uh-setting-row{align-items:center;cursor:pointer;display:grid;gap:.9rem;grid-template-columns:42px 1fr auto;padding:1rem}.uh-setting-row+.uh-setting-row{border-top:1px solid #e5dbcf}.uh-setting-icon{background:#eadfce;border-radius:50%;color:#80603c;display:grid;font-size:1rem;height:40px;place-items:center;width:40px}.uh-setting-icon.is-whatsapp{color:#55745d}.uh-setting-copy{display:grid;gap:.2rem}.uh-setting-copy strong{font:500 1rem Georgia,serif}.uh-setting-copy>span{color:#82715f;font-size:.72rem;line-height:1.45}.uh-switch{align-items:center;display:flex;gap:.45rem}.uh-switch input{height:1px;opacity:0;position:absolute;width:1px}.uh-switch>span{background:#d6c9ba;border-radius:999px;cursor:pointer;height:24px;position:relative;transition:.2s;width:42px}.uh-switch>span:after{background:#fff;border-radius:50%;box-shadow:0 1px 4px #0003;content:'';height:18px;left:3px;position:absolute;top:3px;transition:.2s;width:18px}.uh-switch input:checked+span{background:#8a683f}.uh-switch input:checked+span:after{transform:translateX(18px)}.uh-switch input:focus-visible+span{outline:2px solid #a77d45;outline-offset:2px}.uh-switch em{color:#887866;font-size:.68rem;font-style:normal;min-width:20px}.uh-preference-message{color:#6f6747;font-size:.75rem;margin-top:1rem}
.uh-updates-list{display:grid;gap:.7rem;margin-top:1rem}.uh-update-card{align-items:center;background:#fffaf3;border:1px solid #ded2c4;border-radius:14px;color:inherit;display:grid;gap:.85rem;grid-template-columns:40px 1fr auto;padding:.9rem;text-decoration:none}.uh-update-card:hover{box-shadow:0 8px 22px #5b422312;transform:translateY(-1px)}.uh-update-icon{background:#eadfce;border-radius:50%;color:#80603c;display:grid;height:40px;place-items:center;width:40px}.uh-update-icon.is-whatsapp{color:#55745d}.uh-update-copy{display:grid;gap:.25rem;min-width:0}.uh-update-copy>span{display:flex;gap:1rem;justify-content:space-between}.uh-update-copy strong{font:500 1rem Georgia,serif}.uh-update-copy time,.uh-update-copy small{color:#8b7967;font-size:.66rem}.uh-update-copy p{color:#665646;font-size:.76rem;line-height:1.45;margin:0}.uh-settings-heading{border-top:1px solid #d9cbbb;margin-top:2.2rem;padding-top:1.8rem}
...
@media(max-width:600px){.uh-setting-row{grid-template-columns:38px 1fr}.uh-setting-row>.uh-switch{grid-column:2;justify-self:start;margin-top:.25rem}}
</style>
