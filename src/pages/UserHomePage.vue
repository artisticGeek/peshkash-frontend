<template>
  <div class="uh-page">
    ...
    <main class="uh-shell">
      ...
      <template v-else>
        <nav class="uh-primary-tabs" aria-label="Your Peshkash pages"><RouterLink to="/home/saved" :class="{ active: !isHistoryPage }"><i class="bi bi-collection"></i>Collection</RouterLink><RouterLink to="/home/history" :class="{ active: isHistoryPage }"><i class="bi bi-clock-history"></i>History</RouterLink></nav>
        ...
        <template v-else-if="history && !isHistoryPage">
...
const loginOpen=ref(false),loading=ref(false),error=ref(''),history=ref<History|null>(null),search=ref(''),range=ref<'7d'|'30d'|'90d'|'all'>('30d'),audit=ref<Audit>({rows:[],total:0,page:1,pageSize:20,pages:1}),failedImages=ref(new Set<string>())
let searchTimer:number|undefined
const section=computed(()=>String(route.params.section||'saved') as 'saved'|'liked'|'disliked'|'history')
...
const currentItems=computed<Entry[]>(()=>history.value?history.value[section.value as 'saved'|'liked'|'disliked']:[])
...
async function load(){if(!auth.isLoggedIn)return;loading.value=true;error.value='';try{await loadCollections();if(isHistoryPage.value)await loadAudit(Number(route.query.page)||1)}catch{error.value='We could not load this page right now.'}finally{loading.value=false}}
